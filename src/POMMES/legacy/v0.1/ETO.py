...
def calc_structure_factor(L1, L2, ratio_sampledata,color_line):
    ...
    while (occupancy_Eu1 < (end_occ_Eu1 + step_size)): 
        def intensities_func(h,k,l):
            ...
            f_Eu_approx = approx(24.0063,2.27783,19.9504,0.17353,11.8034,11.6096,3.87243,26.5156,1.36389)
            ...
            def Ta_disp(occEu1):
                ...
                Ta_z = Ta_z_initial + (0.0095/0.50)*(occupancy_Eu1-0.5)
                return Ta_z
            
            def O1_disp(occEu1):
                O1_z_initial = 0.25
                # -0.02008 is film-dependent difference between O1 position at occ = 0.5 and occ = 1.0
                O1_z = O1_z_initial + (-0.02008/0.50)*(occupancy_Eu1-0.5)
                return O1_z
            def O3_disp(occEu1):
                O3_z_initial = 0.25
                # -0.02001 is film-dependent difference between O3 position at occ = 0.5 and occ = 1.0
                O3_z = O3_z_initial + (-0.02001/0.50)*(occupancy_Eu1-0.5)
                return O3_z
            
            # calculate structure factor
            def structure_factor(fEu,fTa,fO,occEu1):
                ...
                Ta1_xyz = [0.5,0.5,Ta_disp(occEu1)]
                Ta2_xyz = [0.5,0.5,1-Ta_disp(occEu1)]
                
                O1_xyz = [0.5,1,O1_disp(occEu1)]
                O2_xyz = [0.5,0.0,1-O1_disp(occEu1)]
                O3_xyz = [0.0,0.5,O3_disp(occEu1)]
                O4_xyz = [1,0.5,1-O3_disp(occEu1)]
                ...
                Eu1_S_term = occEu1 * fEu * math.e ** (factor * (h * Eu1_xyz[0] + k * Eu1_xyz[1] + l * Eu1_xyz[2]))
                Eu2_S_term = (1-occEu1) * fEu * math.e ** (factor * (h * Eu2_xyz[0] + k * Eu2_xyz[1] + l * Eu2_xyz[2]))
                ...
                all_func = [Eu1_S_term,Eu2_S_term,Ta1_S_term,Ta2_S_term,O1_S_term,O2_S_term,O3_S_term,O4_S_term,O5_S_term,O6_S_term]
            ...
            S = structure_factor(f_Eu_approx,f_Ta_approx,f_O_approx,occupancy_Eu1)
        ...
        occ.append(occupancy_Eu1)
        ratios.append(ratio)
        occupancy_Eu1 += step_size
    ...
    ax1.set_xlabel('Occupancy of Eu1 Layer')
